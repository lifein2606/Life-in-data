...
// ========== 产品操作方案 ==========
export const productOperationPlansStorage = {
  async get(productId: string): Promise<OperationPlanInstance[]> {
    const data = await settingsStorage.get<OperationPlanInstance[]>(`product_operation_plans_${productId}`);
    return data || [];
  },
  async set(productId: string, plans: OperationPlanInstance[]): Promise<void> {
    if (plans.length > 0) {
      await settingsStorage.set(`product_operation_plans_${productId}`, plans);
    } else {
      await settingsStorage.delete(`product_operation_plans_${productId}`);
    }
  },
  async delete(productId: string): Promise<void> {
    await settingsStorage.delete(`product_operation_plans_${productId}`);
...
async function productRecordToModel(record: any, sortOrderMap?: Record<string, number>): Promise<Product> {
  ...
  // 尝试从 settings 表读取操作方案数据
  let operationPlans: OperationPlanInstance[] | undefined;
  const settingsOperationPlans = await productOperationPlansStorage.get(record.id);
  if (settingsOperationPlans.length > 0) {
    operationPlans = settingsOperationPlans;
  }
...
export const configStorage = {
  async get(): Promise<GlobalConfig> {
    ...
    try {
      ...
      return {
        ...
        operationPlanTemplates: configMap.get('operation_plan_templates') || DEFAULT_CONFIG.operationPlanTemplates,
      };
    } catch (error) {
  ...
  async set(config: GlobalConfig): Promise<void> {
    ...
    const configs = [
      ...
      { key: 'operation_plan_templates', value: config.operationPlanTemplates },
    ];
...
export const productStorage = {
  ...
  async create(product: Omit<Product, 'id' | 'createdAt' | 'updatedAt' | 'cost'>): Promise<Product> {
    ...
    try {
      ...
      // 将操作方案保存到 settings 表
      if (product.operationPlans && product.operationPlans.length > 0) {
        await productOperationPlansStorage.set(data.id, product.operationPlans);
      }
  ...
  async update(id: string, updates: Partial<Omit<Product, 'id' | 'createdAt' | 'updatedAt' | 'cost'>>): Promise<Product | null> {
    ...
    try {
      ...
      // 处理操作方案
      if (updates.operationPlans !== undefined) {
        await productOperationPlansStorage.set(id, updates.operationPlans);
      }
  ...
  async delete(id: string): Promise<boolean> {
    ...
    try {
      ...
      await productOperationPlansStorage.delete(id);
